...
class ScanFriendTicketViewModelTest {
    ...
    fun testDecodingFailure() = runTest {
        ...
        TestCase.assertEquals("Failed to decode QR Code", viewModel.uiState.value.decodedResult)
        TestCase.assertEquals(ScanFriendQrViewModel.Action.AnalyserError, viewModel.uiState.value.action
        )
    }
    ...
    fun testInvokedAndUpdatedOnce() = runTest {
        userRepository.addUser(mockUser1)
        eventRepository.addEvent(mockEvent1)
        qrCodeAnalyser.onFriendDecoded?.invoke("user1")
    ...
    fun testInvokedAndDenied() = runTest {
        userRepository.addUser(mockUser1)
        eventRepository.addEvent(mockEvent1)
        qrCodeAnalyser.onFriendDecoded?.invoke("user5")
        TestCase.assertEquals(
