...
class EventsOverviewViewModelTest {
    ...
    fun testGetEventsSuccess() = runTest {
        ...
        assert(viewModel.uiState.value.eventList.allEvents == (eventRepository.getEvents() as Resource.Success).data)
        assert(viewModel.uiState.value.eventList.filteredEvents.size == 3)
        assert(viewModel.uiState.value.eventList.filteredEvents == (eventRepository.getEvents() as Resource.Success).data)
        assertNull(viewModel.uiState.value.eventList.selectedEvent)
    }
    ...
    fun testGetUpcomingEventsSuccessWithEvents() = runTest {
       ...
       userRepository.addUser(mockUser)
        ...
        assert(viewModel.uiState.value.eventList.allEvents == (eventRepository.getEventsByIds(listOf("1", "2")) as Resource.Success).data)
        assert(viewModel.uiState.value.eventList.filteredEvents.size == 2)
        assert(viewModel.uiState.value.eventList.filteredEvents == (eventRepository.getEventsByIds(listOf("1", "2")) as Resource.Success).data)
        assertNull(viewModel.uiState.value.eventList.selectedEvent)
